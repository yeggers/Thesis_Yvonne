...
class STDPFedericoFeedForwardCell(torch.nn.Module):
    ...
    def forward1(
        self, x: torch.Tensor, v_decayed: torch.Tensor, max_indices: torch.Tensor, z: torch.Tensor,kernel: torch.Tensor, stride: torch.Tensor, pad:torch.Tensor, device, state: STDPFedericoState
        #self, x: torch.Tensor, z: torch.Tensor, s: int, device, state: STDPFedericoState, 
    ) -> Tuple[torch.Tensor, STDPFedericoState]:
        return stdp_federico_step2(x, v_decayed, max_indices, z, kernel, stride, pad, device, state, shape = self.weight_shape, p=self.p, dt=self.dt)
        #return stdp_federico_step(x, z, s, device, state, shape = self.shape, p=self.p, dt=self.dt)
    ...
    def forward2(
            ...
            #return stdp_federico_step2(x, z, kernel, stride, pad, device, state, shape = self.shape, p=self.p, dt=self.dt)
            return stdp_federico_step(x, z, s, device, state, shape = self.weight_shape, p=self.p, dt=self.dt)
